...
game_core.prototype.update = function (t) {
  ...
  this.dt = this.lastframetime
    ? ((t - this.lastframetime) / 1000.0).fixed()
    : 0.016;
...
game_core.prototype.handle_server_input = function (
  client,
  input,
  input_time,
  input_seq
...
game_core.prototype.client_process_net_prediction_correction = function () {
  ...
  var my_server_pos = this.players.self.host
...
game_core.prototype.client_process_net_updates = function () {
  ...
  if (target && previous) {
    ...
    if (time_point == Infinity) time_point = 0;
    //The most recent server update
    var latest_server_data = this.server_updates[
      this.server_updates.length - 1
    ];
    //These are the exact server positions from this tick, but only for the ghost
    var other_server_pos = this.players.self.host
      ? latest_server_data.cp
      : latest_server_data.hp;
...
game_core.prototype.client_onserverupdate_recieved = function (data) {
  ...
  var player_host = this.players.self.host
    ? this.players.self
    : this.players.other;
  var player_client = this.players.self.host
    ? this.players.other
    : this.players.self;
...
game_core.prototype.client_connect_to_server = function () {
  ...
  this.socket.on(
    "onserverupdate",
    this.client_onserverupdate_recieved.bind(this)
  );
