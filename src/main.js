...
function loadGame(game_UID) {
    ...
    Promise.all(promises).then((results) => {
        ...
        Object.keys(image_map).forEach((key, index) => {
            ...
            if (Array.isArray(image_map[key])) { // so this is an entity with more than one state
    ...
    */
        ...
        // TODO handle error case for when one or more of the promises fails
        Promise.all(img_promises).then((results) => { // TODO Check that each promise returns in the right order
            let images = [];
            results.map((blob, index) => { // note the brackets around image_urls[index]: computed property names
